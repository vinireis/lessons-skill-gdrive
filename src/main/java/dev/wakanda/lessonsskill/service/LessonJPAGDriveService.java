package dev.wakanda.lessonsskill.service;

import java.util.List;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.stereotype.Service;

import com.google.api.services.drive.model.File;

import dev.wakanda.lessonsskill.api.dto.LessonsSkillForms;
import dev.wakanda.lessonsskill.domain.Lesson;
import dev.wakanda.lessonsskill.domain.Skill;
import dev.wakanda.lessonsskill.exception.GDriveException;
import dev.wakanda.lessonsskill.repository.LessonRepository;

@Service
public class LessonJPAGDriveService implements LessonService {
	private static final Logger log = LoggerFactory.getLogger(LessonJPAGDriveService.class);
	private LessonRepository lessonRepository;
	private SkillService skillService;
	private DriveService driveService;

	public LessonJPAGDriveService(LessonRepository lessonRepository, DriveService driveService, SkillService skillService) {
		this.lessonRepository = lessonRepository;
		this.driveService = driveService;
		this.skillService = skillService;
	}

	@Override
	public void saveLessonsBySkill(LessonsSkillForms lessonsSkill) throws GDriveException {
		log.info("Start Lesson Service");
		Skill skill = lessonsSkill.convertToSkill(driveService,skillService);
		log.info("Skill generated by GDrive File ID {}: {}",lessonsSkill.getSkillDriveID(),skill);
		List<File> filesBySkillDriveId = driveService.getFilesBySkillDriveId(lessonsSkill.getSkillDriveID());
		List<Lesson> lessonsByGDriveFile = Lesson.convertToLessons(filesBySkillDriveId,skill);
		this.lessonRepository.saveAll(lessonsByGDriveFile);
		log.info("Lessons by folder: {}",lessonsByGDriveFile);
	}
}
